<template>
  <!-- 水质详情地图 -->
  <div class="map-view">
    <div class="view-item" v-if="btnActive != 0">
    ...
    <div class="l-map-btn">
      <tab-active :btns="btns" @get-btn="getBtn" :activeIndex="btnActive"></tab-active>
    </div>
    <div class="jt-img" v-if="btnActive == 0">
      <img src="@/assets/合成 1_1.gif" />
    </div>
    <div class="map-info" v-if="btnActive == 0">
      <div class="nss">
        <img :src="require('@/assets/map/河湖-警戒.png')" alt="" />
        <span class="name">{{ nssData.siteName }}</span>
        <span class="sz">{{ nssData.szlb }}类</span>
      </div>
      <div class="jlh">
        <img :src="require('@/assets/map/河湖-警戒.png')" alt="" />
        <span class="name">{{ jlhData.siteName }}</span>
        <span class="sz">{{ jlhData.szlb }}类</span>
      </div>
      <div class="yx">
        <img :src="require('@/assets/map/河湖-警戒.png')" alt="" />
        <span class="name">{{ waterQulity.qulity.name }}</span>
        <span class="sz">{{ waterQulity.qulity.qulity }}类</span>
      </div>
      <div class="mmqbz">
        <a v-for="(item, index) in mmqData[10]" v-if="mmqData[10] && mmqData[10].length > 0" :key="index">
          <img v-if="item.rs == 1" :src="require('@/assets/monFlood/1.gif')" />
          <img v-else-if="item.rs == 0" :src="require('@/assets/monFlood/2.gif')" />
          <img v-else="item.rs == 9" :src="require('@/assets/monFlood/9.png')" />
        </a>
        <span class="name">{{ mmqData[1] }}</span>
      </div>
      <div class="bsm">
        <div class="bsmimg">
          <a v-for="(item, index) in bsmData[10]" v-if="bsmData[10] && bsmData[10].length > 0" :key="index">
            <img v-if="item.rs == 1" :src="require('@/assets/monFlood/1.gif')" />
            <img v-else-if="item.rs == 0" :src="require('@/assets/monFlood/2.gif')" />
            <img v-else="item.rs == 9" :src="require('@/assets/monFlood/9.png')" />
          </a>
        </div>
        <span class="name">{{ bsmData[1] }}</span>
      </div>
      <div class="yjxbz">
        <div class="yjximg">
          <a v-for="(item, index) in yjxData[10]" v-if="yjxData[10] && yjxData[10].length > 0" :key="index">
            <img v-if="item.rs == 1" :src="require('@/assets/monFlood/1.gif')" />
            <img v-else-if="item.rs == 0" :src="require('@/assets/monFlood/2.gif')" />
            <img v-else-if="item.rs == 9" :src="require('@/assets/monFlood/9.png')" />
          </a>
        </div>
        <span class="name">{{ yjxData[1] }}</span>
      </div>
      <div class="qszysz">
        <a v-for="(zm, index) in qszData[7]" v-if="qszData[7] && qszData[7].length > 0" :key="index">
          <img v-if="zm.status == 'on'" :src="require('@/assets/flood/3.gif')" alt="zm" />
          <img v-else-if="zm.status == 'off'" :src="require('@/assets/flood/4.gif')" alt="zm" />
        </a>
        <span class="name">{{ qszData[1] }}</span>
      </div>
      <div class="yjclzz1"></div>
      <div class="yjclzz2"></div>
    </div>
    <div class="maptl" v-if="btnActive == 0">
      <div class="maptl-sub">
        <img :src="require('@/assets/map/河湖-警戒.png')" style="width: 18px" alt="" />
        <span>水质点</span>
      </div>
      <div class="maptl-sub">
        <img :src="require('@/assets/monFlood/2.gif')" alt="" />
        <span>泵站</span>
      </div>
      <div class="maptl-sub">
        <img :src="require('@/assets/monFlood/3.gif')" alt="" />
        <span>闸站</span>
      </div>
      <div class="maptl-sub">
        <span class="yjclzz"></span>
        <span>应急处理装置</span>
      </div>
    </div>
    <div class="view-item">
      <panel-view :heading="waterQulity.qulity.mc || waterQulity.qulity.name || waterQulity.qulity.siteName">
        ...
        <div class="amount-item">
          <div class="amount-item-sub" v-for="(item, index) in amountList" :key="index">
            ...
            <div :class="item.class" v-if="waterQulity.qulity.originPage == 'sd' || waterQulity.qulity.originPage == 'zd'">
              <span class="big-font">{{ item.amount }}</span
              ><span>{{ item.dw }} </span>
              <span class="blue-label">{{ item.type }}</span>
...
import { floodApi, overview, riverApi } from "@/api";
...
export default class SiteMap extends Vue {
  ...
  amountList: any[] = [];
  getBtn(index: number) {
    this.btnMapAct = JSON.parse(JSON.stringify(index)) - 1;
  ...
  // 获取站点数据
  async getZdList() {
    let data = await riverApi.getQulityGis();
    this.nssData = data.filter((item: any) => item.siteId == "44")[0];
    this.jlhData = data.filter((item: any) => item.siteId == "48")[0];
  }
  mmqData: any[] = [];
  bsmData: any[] = [];
  yjxData: any[] = [];
  // 获取泵站数据
  async getBzList() {
    let data = await overview.getStationGis();
    this.mmqData = data.filter((item: any) => item[0] == "48")[0];
    this.bsmData = data.filter((item: any) => item[0] == "49")[0];
    this.yjxData = data.filter((item: any) => item[0] == "50")[0];
  }
  qszData: any[] = [];
  // 获取涵闸数据
  async getHzList() {
    let data = await overview.getGateGis();
    this.qszData = data.filter((item: any) => item[0] == "DD000002")[0];
  }
  init() {
  ...
  async getSite() {
    ...
    if (this.waterQulity.qulity.originPage == "zd") {
      this.siteParams.sbid = this.waterQulity.qulity.sbid;
      url = "hjzl/real/realData";
    } else if (this.waterQulity.qulity.originPage == "sd") {
      this.siteParams.zdid = this.waterQulity.qulity.siteId;
      url = "hjzl/real/artificialFirstRealData";
    } else if (this.waterQulity.qulity.originPage == "wscl") {
      url = `jnsw/wsclc/sztb/${this.waterQulity.qulity.bid}`;
    } else if (this.waterQulity.qulity.originPage == "yjzz") {
      url = `jnsw/yjclzz/sztb/${this.waterQulity.qulity.bid}`;
    }
    if (this.waterQulity.qulity.originPage == "zd" || this.waterQulity.qulity.originPage == "sd") {
      ...
      if (Array.isArray(data.yzs)) {
        ...
        for (let key in data) {
          if (["clsl", "nh3", "ph", "cod", "tp"].indexOf(key) != -1)
            arr.push({
              label: key,
              amount: data[key],
              type: key == "clsl" ? "sl" : key == "ph" ? "yz" : "ph",
              class: "blue-label",
            });
        }
        this.amountList = arr;
      }
      // let { datamap, statumap, dwmap,level } = data.data
      ...
      //     class: 'blue-label'
      //   }
      //   item.label = key.split('_')[1]
      //   item.amount = datamap[key]
      //   this.amountList.push(item)
      // }
      if (this.waterQulity.qulity.originPage == "zd") {
        this.amountList.unshift({
          label: "本月水质（平均）",
          amount: data.monthLevel,
          dw: "",
          class: this.calcColor(data.level),
        });
      }
      this.amountList.unshift({
        label: "当前水质",
