      ...
      <div class="top">
        <div class="top-item">
          <div class="top-item-sub">
             <suc-input
              v-model="wsgwParams.construction"
              style="width: 200px"
              placeholder= '建设工程'
            >
            </suc-input>
        ...
        <div class="top-item">
          <el-upload
              ...
              action="api/wjServer/paishui/pneng/import"
              accept=".xls,.xlsx"
...
import { wsgwModal, DeleteModal } from "./model";
...
export default class Psgw extends Vue {
  ...
  exportExcel() {
    let url =
      "/api/wjServer/paishui/pneng/export?construction=" +
      this.wsgwParams.construction 
  ...
  async editInfo(params: any) {
    let url = "wjServer/paishui/pneng";
    let detailParams = {
      id: params,
    };
  ...
  // 列表
  async onpage() {
    this.wsgwParams.pageNumber = this.pageParams.current;
    this.wsgwParams.pageSize = this.pageParams.pageSize;
    let url = "wjServer/paishui/pneng/page";
    let data = await publicApi.getWithParam(url, this.wsgwParams);
