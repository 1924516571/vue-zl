    ...
    <div class="info-view bg-shadow">
      <div class="top">
        <div class="top-item">
            ...
            <!-- 行政区域 -->
            <suc-select
              v-model="tsbzParams.areaName"
              :options="areaOptions"
              style="width: 200px"
              :config="{ placeholder: '行政区域', clearable: false }"
            >
            </suc-select>
          </div>
          <div class="top-item-sub">
             <suc-input
              v-model="tsbzParams.name"
              style="width: 200px"
              placeholder= '泵站名称'
            >
            </suc-input>
...
import { tsbzModal, DeleteModal } from "./model";
...
export default class Enterprise extends Vue {
  ...
  async getBb() {
    let url = "wjServer/paishui/pump/areas";
  ...
  exportExcel() {
    let url =
      ...
      "/api/wjServer/paishui/pump/export?name=" +
      this.tsbzParams.name +
      "&areaName=" +
      this.tsbzParams.areaName
  ...
  // 列表
  async onpage() {
    this.tsbzParams.pageNumber = this.pageParams.current;
    this.tsbzParams.pageSize = this.pageParams.pageSize;
    let url = "wjServer/paishui/pump/page";
    let data = await publicApi.getWithParam(url, this.tsbzParams);
