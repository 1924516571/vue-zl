<template>
  ...
  <el-dialog :title="title" :visible.sync="model" close-on-click-modal:false :before-close="onCancel" v-if="model">
    <el-form :model="formParams" :rules="rules" ref="zcglForm" label-width="auto" size="small">
      <el-form-item label="污水处理厂" v-if="activeIndex == 0">
        <el-input v-model="formParams.companyname"></el-input>
      </el-form-item>
      <el-form-item label="泵站名称" v-if="activeIndex == 1">
        <el-input v-model="formParams.pumpname"></el-input>
      </el-form-item>
      <el-form-item label="设备名称">
        <el-input v-model="formParams.devicename"></el-input>
      </el-form-item>
      <el-form-item label="设备规格型号">
        <el-input v-model="formParams.model"></el-input>
      </el-form-item>
      <el-form-item label="设备维护情况">
        <el-input v-model="formParams.info"></el-input>
      </el-form-item>
      <el-form-item label="维护时间">
        <el-input v-model="formParams.tm"></el-input>
      </el-form-item>
...
export default class Wscl extends Vue {
  ...
  onOk() {
    (this.$refs["zcglForm"] as any).validate((valid: any) => {
      if (valid) {
        this.onSubmit();
      } else {
        return false;
  ...
  onCancel() {
    this.toggle(false);
    (this.$refs["zcglForm"] as any).resetFields();
  }
