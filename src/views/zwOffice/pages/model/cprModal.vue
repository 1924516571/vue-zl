<template>
  <el-dialog :append-to-body="true" :title="title" :visible.sync="modelcpr" :close-on-click-modal="true" :before-close="onCancel">
    <el-form :model="cprParams" ref="cprForm" label-width="auto" size="small">
      <div>
...
export default class cprModal extends Vue {
  @Prop() title!: string;
  @Prop() modelcpr!: boolean; //控制弹框显隐
  @Prop() loading!: boolean;
  @Prop({ type: Object, default: () => {} })
  cprParams!: any; //新增接口需要的参数
  tableData: any[] = [];
  resultData: any[] = [];
  ...
  onCheck(row:any) {
    if (row.status){
      // console.log("选择了一行")
      this.cprParams.activeUserIds.push(row.id);
      this.resultData.push(row);

    } else {
      // console.log("取消了一行")
      this.cprParams.activeUserIds = this.cprParams.activeUserIds.filter((item: any) => item != row.id)
   ...
   async initResult(total:any) {
      ...
      if (data.result == true) {
        ...
        for (let i = 0; i < apiData.length; i++) {
          if(this.cprParams.activeUserIds.includes(apiData[i].id)){
   ...
   async onpage() {
      ...
      if (data.result == true) {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          if(this.cprParams.activeUserIds.includes(this.tableData[i].id)){
