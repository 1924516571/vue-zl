      ...
      <div class="list-container">
        ...
        >
          <div class="list-item-site">
            ...
            <div :class="[list.state == '异常' ? 'state-red' : 'state-green']">
              <span style="color:#fff;">{{ list.state }}</span>
            </div>
          </div>
          <div class="list-item-info">
            <div class="info-name"><span>所属片区</span></div>
            <div class="info-value"><span v-text="list.area">A片区</span></div>
            <div class="info-value">
              <span>当前水位 </span
              ><span class="state-blue" v-text="list.level">1.5m</span>
            </div>
            ...
            <div class="info-value">
              <!-- <span>1h水位变化：</span> -->
              <span
                :class="[list.state == '异常' ? 'state-red-txt' : 'state-blue']"
                v-text="list.level1"
                >+0.00m</span>
            </div>
...
export default class MonitorList extends Vue {
  ...
  async getMonitor () {
    let that = this
    that.listArr = []
    const monitorData = await networkApi.getMonitor(that.monitorParams)
    let { data } = monitorData
    if (data.length) {
      that.listArr = data.map((item: any) => {
        return {
          site: item.SBMC,
  ...
  async getSbid () {
    let that = this
    let idParams = {
      yjId: that.netWork.row.YJID
    }
    const sbidData = await networkApi.getOverviewData(idParams)
    let { upstreamdevice } = sbidData

    let sbids = upstreamdevice.map((item: any) => {
      return item.sbid
    })
