    ...
    <div class="weather">
      <div class="weather-item">
        <div class="status">
          ...
          <div>
            <div><span>{{dataList.temp}}</span><span>{{dataList.minTemp == 9999?'':dataList.minTemp}}~{{dataList.maxTemp == 9999?'':dataList.maxTemp}}℃</span></div>
            <div><span>{{dataList.realTemp == 9999?'':dataList.realTemp}}</span><span>℃</span><span>当前</span></div>
          </div>
        </div>
        <div class="time">
          <div><span>{{dataList.nowWeek}}</span><span style="margin-left: 10px">{{dataList.air}}</span></div>
          <div><span>{{dataList.date}} {{dataList.time}}</span></div>
        </div>
      </div>
    </div>
  </div>
...
export default class ModuleView extends Vue {
  ...
  async getWeather() {
    ...
    if(this.dataList.publish_time){
        this.dataList.date = this.$utils.dateFormat(new Date(this.dataList.publish_time), 'yyyy/MM/dd');
        this.dataList.time = this.$utils.dateFormat(new Date(this.dataList.publish_time), 'HH:mm');
        let week = "星期" + "日一二三四五六".charAt(new Date(this.dataList.publish_time).getDay());
        ...
        this.dataList.src = require('@/assets/icon/' + this.dataList.weather.info + '.png');
    }
  }
}
</script>
