		...
		<div class="content">
          ...
          >
            ...
            >
				...
				<!-- <suc-form-item :config="{ label: '防汛分组：' }">
          <suc-select
            style="width: 98%"
            v-model="formParams.fzid"
            :config="{
              clearable: true,
              filterable: true,
              placeholder: '防汛分组',
...
export default class CreateTask extends Vue {
	...
	// pqId: string = "";
	coordstring: string = "";
	// fxfzOptions: SelectOptions = []; //防汛分组
	// 负责人
	// async getFxfz() {
	//   this.fxfzOptions = [];
	//   let data = await taskApi.getFxfz();
	//   this.fxfzOptions = data.map((e: any) => {
	//     return {
	//       value: e[0],
	//       label: e[1],
	//     };
	...
	async setTask() {
		...
		url = "jnsw/newtask/floodPreventionTask";
