    ...
    <div class="content scroll">
      <el-form :model="formParams" ref="taskForm" label-width="auto" :rules="rules" size="small">
        ...
        <el-form-item label="对象类型" prop="engType">
          <suc-select v-model="formParams.engType" :options="objectTypeData" :config="{ placeholder: '对象类型', clearable: false, filterable: true }" style="width:100%"></suc-select>
        </el-form-item>
        ...
        <el-form-item label="对象名称" prop="objectName">
          <div class="view-action-box">
            <el-select popper-class="elselect" v-model="formParams.objectName" value-key="item" placeholder="请选择" @change="handleCommissioer" clearable filterable style="width:100%">
              <el-option v-for="(item, idx) of engData" :key="idx" :label="item.sectionname" :value="item" />
            </el-select>
          </div>
        </el-form-item>
...
export default class CreateTask extends Vue {
  ...
  async getYhobj() {
    ...
    if (data.result == true) {
      // this.engData = data.data.map((item: any) => {
      //   return {
  ...
  async handleCommissioer(val: any) {
    this.formParams.objectName = val.sectionname;
    this.formParams.object = val.id;
    this.formParams.area = "cz,czwj";
    this.formParams.lat = val.latitude;
    this.formParams.lon = val.longitude;
  }
