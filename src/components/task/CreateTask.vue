    ...
    <div class="content scroll">
      <el-form :model="formParams" ref="taskForm" label-width="auto" :rules="rules" size="small">
        <el-form-item label="任务名称" prop="problemName">
          <suc-input @on-change="getInput" v-model="formParams.problemName" placeholder="请输入任务名称" style="width: 98%"> </suc-input>
        </el-form-item>
        ...
        <el-form-item label="养护内容">
          <suc-input v-model="formParams.yhmbId" style="width: 98%"> </suc-input>
        </el-form-item>
        <el-form-item label="位置">
          <suc-input style="width: 98%" v-model="formParams.address"> </suc-input>
        </el-form-item>
        <el-form-item label="任务描述">
          <suc-input style="width: 98%" v-model="formParams.content" type="textarea"> </suc-input>
        </el-form-item>
...
import { create } from "../../api/environment/transferFa";
...
export default class CreateTask extends Vue {
  ...
  getAreaCode() {
    publicApi.getAreaCode().then((res: any) => {
      if (res.result) {
  ...
  getTreeData(param: any) {
    for (var i = 0; i < param.length; i++) {
      if (param[i].children.length < 1) {
        param[i].children = undefined;
      } else {
        this.getTreeData(param[i].children);
      }
    }
    return param;
  }
  ...
  //   const s = a.toString();
  //   this.yhContent = s;
  //   this.$set(this.formParams, "yhContent", s);
  // }
