<template>
  <div class="main-map">
    <suc-map ref="mapObject" :options="olMap.map" v-bind.sync="olMap.map.view" @singleclick="mapSingClick" @singleclick-blank="mapSingClick">
      ...
      <slot name="wsmapbox" :zoom="zoom"></slot>
      <slot name="gwmap"></slot>
    ...
    <slot name="tool-btn">
      <div class="measure-btn">
        <div class="measure-item" @click="onWqShow">
          <span>圩区</span>
        </div>
        <div class="measure-item" @click="onGwShow">
          <span>管网</span>
        </div>
        <div class="measure-item" @click="onSxShow">
          <span>水系</span>
        </div>
...
export default class MapManage extends Vue {
  ...
  // 圩区图层展示
  onWqShow() {
    this.olMap.wjLayer[0].visible = !this.olMap.wjLayer[0].visible;
  }
  // 管网图层展示
  onGwShow() {
    this.olMap.wjLayer[4].visible = !this.olMap.wjLayer[4].visible;
    this.olMap.wjLayer[5].visible = !this.olMap.wjLayer[5].visible;
    if (this.olMap.wjLayer[4].visible || this.olMap.wjLayer[5].visible) {
      this.olMap.wjLayer[2].visible = false;
    }
  }
  // 水系
  onSxShow() {
    this.olMap.wjLayer[2].visible = !this.olMap.wjLayer[2].visible;
    if (this.olMap.wjLayer[2].visible) {
      this.olMap.wjLayer[4].visible = false;
      this.olMap.wjLayer[5].visible = false;
    }
  }
}
</script>
