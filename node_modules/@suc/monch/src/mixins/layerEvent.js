import {coordinateTransform, removeOverlay, setLabelEvent, transformStyle} from "../utils/helpers";
...
export default {
    ...
    methods: {
        ...
        listenerMove(data) {
            const vm = this;
            if (data.olLayer && data.olLayer !== vm.layer) {
                this.listenerMove_blank();
                // console.log(this.listenerMove_blank(),"天天");
                return;
            }
            // 拿到鼠标移动上去的obj
            const feature = data.feature;
            // console.log(feature,"哈哈哈哈");
            //恢复前次标记的feature的样式
            // console.log(vm.curMoveNormalStyle,"弟弟");
            ...
            */
            ...
            //为feature设置悬浮样式
            const moveStyle = vm.moveStyle || (feature.get('featureInfo')&&feature.get('featureInfo').data.moveStyle);
            // console.log(feature.get('featureInfo'),"昨天");
            // console.log(moveStyle,"55555");
            ...
            //名称弹框
            let name = feature.get('featureInfo')&&feature.get('featureInfo').data.name;
        ...
        listenerClick(data) {
            ...
            const clickStyle = vm.clickStyle || (feature.get('featureInfo')&&feature.get('featureInfo').data.clickStyle);
        ...
        listenOther(map) {  // 鼠标事件发生在其他图层上
            const vm = this;
            const clickKey = map.on('singleclick', function (event) {
                let layerFeature = map.forEachFeatureAtPixel(event.pixel, function (feature, olLayer) {
                    return [olLayer, feature];
                });
                if (layerFeature && layerFeature[0] !== vm.layer) {
                    vm.listenerClick_blank();
                }
            })
            const moveKey = map.on('pointermove', function (event) {
                let layerFeature = map.forEachFeatureAtPixel(map.getEventPixel(event.originalEvent), function (feature, olLayer) {
                    return [olLayer, feature];
                });
                if (layerFeature && layerFeature[0] !== vm.layer) {
                    vm.listenerMove_blank();
                }
            })

            vm.$once("hooke:beforeDestroy", function () {
                unByKey(clickKey);
                unByKey(moveKey);
            })
        },
